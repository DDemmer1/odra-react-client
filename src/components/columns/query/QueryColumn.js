...
class QueryColumn extends Component {
    ...
    componentDidMount() {
        axios.get(this.props.apiUrl + "articles/source/" + this.props.column.source + "?limit="+this.state.limit+"&query=" + this.props.column.query)
            .then((result) => that.setState({articles: result.data}));
        //start 5sec refresh
        var that = this;
        window.setInterval(function () {
                axios.get(that.props.apiUrl + "articles/source/" + that.props.column.source + "?limit="+that.state.limit+"&query=" + that.props.column.query)
                    .then((result) => that.setState({articles: result.data}));
            }, 15000
        );
    }
